...
export interface FerrofluidProps {
  ...
  mixBlendMode?: string;
  /**
   * Explicit background override (any CSS color/gradient). Takes precedence
   * over `backgroundLight`/`backgroundDark` and theme detection entirely.
   */
  background?: string;
  /** Background used when the resolved theme is light. Defaults to 'transparent'. */
  backgroundLight?: string;
  /** Background used when the resolved theme is dark. Defaults to 'transparent'. */
...
// Mirrors the app-wide theme mechanism driven by ThemeToggle, which sets
// document.documentElement[data-theme] and falls back to the OS preference.
...
const useResolvedTheme = (): ResolvedTheme => {
  ...
  useEffect(() => {
    // Reading data-theme/matchMedia must happen client-side only (SSR has
    // no access to either), so the initial theme can only be known once
...
const Ferrofluid: React.FC<FerrofluidProps> = ({
  ...
  const resolvedBackground = background ?? (resolvedTheme === 'dark' ? backgroundDark : backgroundLight);
